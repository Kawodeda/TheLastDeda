...
public class Config
{
    ...
    {
        for (Path configPath: new Path[] { LsoCompat.modConfigPath, LsoCompat.modConfigJsons })
        {
            try {
                Files.createDirectory(configPath);
            } catch (FileAlreadyExistsException ignored) {
            } catch (IOException e) {
                LsoCompat.LOGGER.error("Failed to create LSO Compat config directory " + configPath);
                e.printStackTrace();
            }
        }

        context.registerConfig(ModConfig.Type.COMMON, SPEC, LsoCompat.MODID + "/" + LsoCompat.MODID + "-common.toml");

        JsonConfigRegistration.init(LsoCompat.modConfigJsons.toFile());
    }
